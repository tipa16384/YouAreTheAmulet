    ...
    def game_loop(self):
        ...
        while running:
            ...
            for i, line in enumerate(self.intro_lines):
                xparency = 255 - ((elapsed_time - self.scroll_speed * i)/ self.scroll_speed) * 255
                xparency = 255 - max(0, min(255, xparency))
                line.set_alpha(xparency)
                self.screen.blit(line, (self.screen.get_width()/2 - line.get_width()/2, start_y + (i * line_height)))
